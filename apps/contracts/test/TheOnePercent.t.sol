...
contract TheOnePercentTest is Test {
    TheOnePercent public theOnePercent;
    ...
    function setUp() public {
        ...
        theOnePercent = new TheOnePercent(mockVerificationHub, mockScope, mockConfigId);
    }
    ...
    function test_StakeForPoolCreation_MinimumStake() public {
        ...
        theOnePercent.stakeForPoolCreation{value: 5 ether}();
        
        (, , uint256 poolsRemaining, , ) = theOnePercent.getCreatorInfo(creator1);
    ...
    function test_StakeForPoolCreation_MaximumStake() public {
        ...
        theOnePercent.stakeForPoolCreation{value: 50 ether}();
    ...
    function test_StakeForPoolCreation_RevertBelowMinimum() public {
        ...
        theOnePercent.stakeForPoolCreation{value: 4 ether}();
    ...
    function test_StakeForPoolCreation_RevertAboveMaximum() public {
        ...
        theOnePercent.stakeForPoolCreation{value: 51 ether}();
    ...
    function test_CalculatePoolsEligible() public {
        assertEq(theOnePercent.calculatePoolsEligible(5 ether), 1);
        assertEq(theOnePercent.calculatePoolsEligible(10 ether), 2);
        assertEq(theOnePercent.calculatePoolsEligible(15 ether), 3);
        assertEq(theOnePercent.calculatePoolsEligible(25 ether), 5);
        assertEq(theOnePercent.calculatePoolsEligible(50 ether), 10);
    }
    ...
    function test_CreatePool_Success() public {
        ...
        theOnePercent.createPool(1 ether, 4);
        
        (address creator, uint256 entryFee, uint256 maxPlayers, uint256 currentPlayers, uint256 prizePool, TheOnePercent.PoolStatus status, , ) = 
            theOnePercent.getPoolInfo(1);
        ...
        assertTrue(status == TheOnePercent.PoolStatus.OPENED);
    ...
    function test_CreatePool_RevertNoRemainingPools() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 5 ether}(); // Only 1 pool
        
        theOnePercent.createPool(1 ether, 4); // Create first pool
        
        vm.expectRevert("No remaining pools available");
        theOnePercent.createPool(1 ether, 4); // Try to create second pool
    ...
    function test_CreatePool_RevertInvalidParameters() public {
        ...
        theOnePercent.createPool(0, 4);
        
        vm.expectRevert("Pool must have at least 2 players");
        theOnePercent.createPool(1 ether, 1);
    ...
    function test_JoinPool_Success() public {
        ...
        theOnePercent.joinPool{value: 1 ether}(1);
        
        (, , , uint256 currentPlayers, uint256 prizePool, , , ) = theOnePercent.getPoolInfo(1);
    ...
    function test_JoinPool_RevertIncorrectEntryFee() public {
        ...
        theOnePercent.joinPool{value: 0.5 ether}(1);
        vm.stopPrank();
    }
    ...
    function test_JoinPool_AutoActivateWhenFull() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 2); // 2-player pool
        vm.stopPrank();
        ...
        vm.prank(player2);
        theOnePercent.joinPool{value: 1 ether}(1); // This should activate the pool
        
        (, , , , , TheOnePercent.PoolStatus status, , ) = theOnePercent.getPoolInfo(1);
        assertTrue(status == TheOnePercent.PoolStatus.ACTIVE);
        
        assertEq(theOnePercent.getCurrentRound(1), 1);
    }
    
    function test_JoinPool_ActivateAt50Percent() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 4); // 4-player pool, 50% = 2 players
        vm.stopPrank();
        ...
        (, , , , , status, , ) = theOnePercent.getPoolInfo(1);
        assertTrue(status == TheOnePercent.PoolStatus.OPENED);
        
        // But with 2 players (50%), pool should be manually activatable
        assertTrue(theOnePercent.canActivatePool(1));
        
        // Manual activation by creator
        vm.prank(creator1);
        theOnePercent.activatePool(1);
    ...
    function test_GameFlow_MinorityWins_Success() public {
        ...
        theOnePercent.joinPool{value: 1 ether}(1);
        vm.prank(player4);
        theOnePercent.joinPool{value: 1 ether}(1); // Pool auto-activates
        
        // Round 1: 1 HEADS, 3 TAILS -> HEADS should win (minority)
        vm.prank(player1);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.HEADS);
        vm.prank(player2);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.TAILS);
        vm.prank(player3);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.TAILS);
        vm.prank(player4);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.TAILS); // Auto-resolves round
        
        // Check round 1 results
        address[] memory remainingPlayers = theOnePercent.getRemainingPlayers(1);
        assertEq(remainingPlayers.length, 1);
        assertEq(remainingPlayers[0], player1); // Only HEADS player should remain
        assertEq(theOnePercent.getCurrentRound(1), 1); // Game completed in round 1
        
        // Check game completion
        (, , , , , TheOnePercent.PoolStatus status, , ) = theOnePercent.getPoolInfo(1);
        assertTrue(status == TheOnePercent.PoolStatus.COMPLETED);
    }
    ...
    function test_GameFlow_MultipleRounds() public {
        ...
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 6);
        vm.stopPrank();
        ...
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.TAILS);
        vm.prank(player6);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.TAILS); // Auto-resolves
        
        // Check round 1 results
        address[] memory remainingPlayers = theOnePercent.getRemainingPlayers(1);
        assertEq(remainingPlayers.length, 2); // player1 and player2 should remain
        assertEq(theOnePercent.getCurrentRound(1), 2);
        
        // Round 2: 1 HEADS, 1 TAILS -> Tie, random resolution
        vm.prank(player1);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.HEADS);
        vm.prank(player2);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.TAILS); // Auto-resolves with tie-breaker
        
        // Check final result
        remainingPlayers = theOnePercent.getRemainingPlayers(1);
    ...
    function test_MakeSelection_RevertInvalidChoice() public {
        ...
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 2);
        vm.stopPrank();
        ...
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.NONE);
        vm.stopPrank();
    }
    ...
    function test_UnstakeAndClaim_EarlyUnstake_Penalty() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 4); // Create one pool but don't complete it
        ...
        assertTrue(theOnePercent.isPoolAbandoned(1));
        
        // Creator should no longer have active stake
        (, , , bool hasActiveStake, ) = theOnePercent.getCreatorInfo(creator1);
        assertFalse(hasActiveStake);
    }
    
    function test_UnstakeAndClaim_AllPoolsCompleted_NoRenalty() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}(); // 2 pools
        theOnePercent.createPool(1 ether, 2);
        theOnePercent.createPool(1 ether, 2);
        vm.stopPrank();
        ...
        vm.prank(player3);
        theOnePercent.joinPool{value: 1 ether}(2);
        vm.prank(player4);
        theOnePercent.joinPool{value: 1 ether}(2);
        vm.prank(player3);
        theOnePercent.makeSelection(2, TheOnePercent.PlayerChoice.HEADS);
        vm.prank(player4);
        theOnePercent.makeSelection(2, TheOnePercent.PlayerChoice.TAILS);
    ...
    function test_ViewFunctions() public {
        // Setup game
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 15 ether}(); // 3 pools
        theOnePercent.createPool(1 ether, 4);
        vm.stopPrank();
        ...
        theOnePercent.joinPool{value: 1 ether}(1);
        vm.prank(player4);
        theOnePercent.joinPool{value: 1 ether}(1); // Pool activates
        
        // Test view functions before choices
        assertEq(theOnePercent.getCurrentRound(1), 1);
        assertEq(theOnePercent.getRemainingPlayers(1).length, 4);
        assertFalse(theOnePercent.hasPlayerChosen(1, player1));
        assertFalse(theOnePercent.isPlayerEliminated(1, player1));
        
        // Make some choices
        vm.prank(player1);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.HEADS);
        
        assertTrue(theOnePercent.hasPlayerChosen(1, player1));
        assertEq(uint(theOnePercent.getPlayerChoice(1, player1)), uint(TheOnePercent.PlayerChoice.HEADS));
        ...
        assertTrue(theOnePercent.isPlayerEliminated(1, player2));
        assertTrue(theOnePercent.isPlayerEliminated(1, player3));
        assertTrue(theOnePercent.isPlayerEliminated(1, player4));
        assertFalse(theOnePercent.isPlayerEliminated(1, player1));
        
        (uint256 currentRound, uint256 remainingCount, uint256 totalCount, bool isComplete) = 
            theOnePercent.getGameProgress(1);
    ...
    function test_EdgeCase_PoolActivation_ManualActivation() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 4); // 4-player pool, need 2 for 50%
        vm.stopPrank();
        
        vm.prank(player1);
        theOnePercent.joinPool{value: 1 ether}(1);
        vm.prank(player2);
        theOnePercent.joinPool{value: 1 ether}(1); // 50% capacity reached
    ...
    function test_EdgeCase_NonExistentPool() public {
        ...
        theOnePercent.joinPool{value: 1 ether}(999);
    ...
    function test_EarlyUnstake_AbandonOpenedPools() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 4); // Pool 1 - OPENED
        theOnePercent.createPool(1 ether, 4); // Pool 2 - OPENED
        vm.stopPrank();
        ...
        assertTrue(theOnePercent.isPoolAbandoned(1));
        assertTrue(theOnePercent.isPoolAbandoned(2));
        ...
        // Check pool status
        (, , , , , TheOnePercent.PoolStatus status1, , ) = theOnePercent.getPoolInfo(1);
        (, , , , , TheOnePercent.PoolStatus status2, , ) = theOnePercent.getPoolInfo(2);
        assertTrue(status1 == TheOnePercent.PoolStatus.ABANDONED);
        assertTrue(status2 == TheOnePercent.PoolStatus.ABANDONED);
    }
    
    function test_EarlyUnstake_ProtectActiveGameplay() public {
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 15 ether}(); // 3 pools
        theOnePercent.createPool(1 ether, 4); // Pool 1 - will be OPENED
        theOnePercent.createPool(1 ether, 4); // Pool 2 - will be ACTIVE
        vm.stopPrank();
        ...
        theOnePercent.joinPool{value: 1 ether}(2);
        vm.prank(player5);
        theOnePercent.joinPool{value: 1 ether}(2); // Pool 2 becomes ACTIVE
        ...
        (, , , , , TheOnePercent.PoolStatus status2, , ) = theOnePercent.getPoolInfo(2);
        assertTrue(status2 == TheOnePercent.PoolStatus.ACTIVE);

        // Pool 2 creator should be transferred to contract
        (address pool2Creator, , , , , , , ) = theOnePercent.getPoolInfo(2);
        assertEq(pool2Creator, address(theOnePercent)); // Contract is now the creator
    ...
    function test_EarlyUnstake_CreatorLosesRewardsFromTransferredPools() public {
        ...
        theOnePercent.joinPool{value: 1 ether}(1);
        vm.prank(player2);
        theOnePercent.joinPool{value: 1 ether}(1); // Pool becomes ACTIVE
        ...
        uint256 creatorReward = theOnePercent.calculateCreatorReward(creator1);
    ...
    function test_ManualRefundFromAbandonedPool() public {
        ...
        vm.prank(player1);
        theOnePercent.claimRefundFromAbandonedPool(1);
    }
    ...
    function test_ManualRefundFromAbandonedPool_NotParticipant() public {
        ...
        vm.prank(creator1);
        theOnePercent.unstakeAndClaim(); // Abandon pool
    ...
    function test_AbandonedPoolCreatorFeesGoToProjectPool() public {
        ...
        (address poolCreator, , , , , , , ) = theOnePercent.getPoolInfo(1);
        assertEq(poolCreator, address(theOnePercent));
        ...
        assertEq(theOnePercent.getProjectPoolBalance(), projectPoolBefore + expectedCreatorFee);
    }

    // PROJECT POOL MANAGEMENT TESTS

    function test_ProjectPoolBalance_InitiallyZero() public {
        assertEq(theOnePercent.getProjectPoolBalance(), 0);
    }
    ...
    function test_WithdrawProjectPoolFunds_Success() public {
        ...
        uint256 projectPoolBalance = theOnePercent.getProjectPoolBalance();
        ...
        theOnePercent.withdrawProjectPoolFunds(withdrawAmount);

        assertEq(theOnePercent.getProjectPoolBalance(), projectPoolBalance - withdrawAmount);
    ...
    function test_WithdrawProjectPoolFunds_RevertNotOwner() public {
        ...
        vm.prank(creator1);
        theOnePercent.unstakeAndClaim(); // Create penalty

        // Non-owner tries to withdraw
        vm.expectRevert();
        vm.prank(player1);
        theOnePercent.withdrawProjectPoolFunds(1 ether);
    }
    ...
    function test_WithdrawProjectPoolFunds_RevertZeroAmount() public {
        ...
        theOnePercent.withdrawProjectPoolFunds(0);
    }
    ...
    function test_VerificationBonus_UnverifiedCreator() public {
        ...
        (, , uint256 poolsRemaining, , bool isVerified) = theOnePercent.getCreatorInfo(creator1);
    ...
    function test_VerificationBonus_VerifiedCreator() public {
        ...
        assertTrue(theOnePercent.calculatePoolsEligible(10 ether, creator1) == 2); // Unverified = 2 pools
        ...
        assertEq(theOnePercent.calculatePoolsEligible(5 ether, creator1), 1);
        assertEq(theOnePercent.calculatePoolsEligible(10 ether, creator1), 2);
        assertEq(theOnePercent.calculatePoolsEligible(25 ether, creator1), 5);
    ...
    function test_VerificationStatus_Functions() public {
        ...
        assertFalse(theOnePercent.isCreatorVerified(creator1));
        assertEq(theOnePercent.getVerificationBonus(creator1), 0);

        (bool isVerified, uint256 bonusPools, string memory status) = theOnePercent.getVerificationStatus(creator1);
        assertFalse(isVerified);
    ...
    function test_PreviewPoolsEligible() public {
        ...
        (uint256 basePools, uint256 totalPools, uint256 bonusPools) =
            theOnePercent.previewPoolsEligible(15 ether, creator1);
    ...
    function test_CalculatePoolsEligible_WithoutVerification() public {
        ...
        assertEq(theOnePercent.calculatePoolsEligible(10 ether, creator1), 2);
        assertEq(theOnePercent.calculatePoolsEligible(25 ether, creator1), 5);
        assertEq(theOnePercent.calculatePoolsEligible(50 ether, creator1), 10);
    ...
    function test_UnanimousSelection_AllHeads_RoundRepeats() public {
        ...
        vm.startPrank(creator1);
        theOnePercent.stakeForPoolCreation{value: 10 ether}();
        theOnePercent.createPool(1 ether, 3);
        vm.stopPrank();
        ...
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.HEADS);
        vm.prank(player3);
        theOnePercent.makeSelection(1, TheOnePercent.PlayerChoice.HEADS); // Auto-resolves round
        ...
        assertFalse(theOnePercent.isPlayerEliminated(1, player1));
        assertFalse(theOnePercent.isPlayerEliminated(1, player2));
        assertFalse(theOnePercent.isPlayerEliminated(1, player3));
        ...
        assertFalse(theOnePercent.hasPlayerChosen(1, player1));
        assertFalse(theOnePercent.hasPlayerChosen(1, player2));
        assertFalse(theOnePercent.hasPlayerChosen(1, player3));
    }
    ...
    function test_UnanimousSelection_AllTails_RoundRepeats() public {
        ...
        assertFalse(theOnePercent.hasPlayerChosen(1, player2));
        assertFalse(theOnePercent.hasPlayerChosen(1, player3));
        assertFalse(theOnePercent.hasPlayerChosen(1, player4));
    }
    ...
    function test_UnanimousSelection_ThenNormalElimination() public {
        ...
        assertEq(theOnePercent.getRemainingPlayers(1).length, 4);
        assertEq(theOnePercent.getCurrentRound(1), 1); // Round should not advance
